Monitoramento e Visualização de Desempenho - SeparacaoApp

Este é um aplicativo web desenvolvido em Python 3 utilizando o Streamlit, com integração ao banco de dados Firebird por meio do Firebird-Driver. O objetivo principal do app é realizar o monitoramento em tempo real dos processos da tela SeparacaoApp, que pode ser encontrado no repositório [Meu Repositório].

Além de monitorar os pedidos, o aplicativo fornece visualizações dinâmicas de desempenho com gráficos interativos criados com Plotly Express, oferecendo insights valiosos para a otimização de processos.
Funcionalidades

O aplicativo possui três abas principais:

- Monitoramento de Pedidos: Acompanhamento em tempo real do status dos pedidos e processos.
- Auditoria: Registro e análise das atividades realizadas, promovendo transparência e rastreabilidade.
- Dashboard: Visualização gráfica do desempenho do sistema, com filtros personalizados para facilitar a interpretação dos dados.

Filtros Personalizados

Cada aba conta com filtros específicos que permitem ao usuário customizar a visualização de acordo com suas necessidades, tornando a navegação mais intuitiva e a análise mais eficiente.
